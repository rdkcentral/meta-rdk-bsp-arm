...
#
# DESCRIPTION
# This is a modified version of the 'bootimg-efi' source plugin class
# that includes boot files for the Raspberry Pi, for a 
# 'hybrid' setup that chains to EFI boot.
#
# AUTHORS
# Mathew McBride <matt (at] traverse.com.au>
# modifed slightly from 'bootimg-efi.py' from:
...
class BootimgRPIPlusEFIPlugin(SourcePlugin):
    """
    ...
    @classmethod
    def _copy_additional_files(cls, hdddir, initrd, dtb, source_params):
        ...
        rpi_files_dir = source_params.get('extra_files')
        if rpi_files_dir:
            logger.info("Copying from BOOTFILES_DIR_NAME (%s)" % (rpi_files_dir))
            cp_cmd = "cp -r %s/* %s" % (rpi_files_dir, hdddir)
            exec_cmd(cp_cmd, True)
        else:
            logger.info("No BOOTFILES_DIR_NAME ({})".format(rpi_files_dir))
    ...
    def do_configure_grubefi(cls, hdddir, creator, cr_workdir, source_params):
        ...
        cls._copy_additional_files(hdddir, initrd, dtb, source_params)
